...
public class MqAspect {
    ...
    @ConditionalOnClass(name="com.akuchen.platform.components.mq.annotations.AlMQConsumer")
    public AspectJExpressionPointcutAdvisor mqAdvisor(){
        AspectJExpressionPointcutAdvisor advisor = new AspectJExpressionPointcutAdvisor();
        advisor.setExpression("@annotation(com.akuchen.platform.components.mq.annotations.AlMQConsumer)");
    ...
    private MethodInterceptor initMethodInterceptor(){
        MethodInterceptor interceptor = pjp -> {
            ...
            try {
                ...
                traceLogManager.info("[trace-mq][{}]【request={}】【response={}】[cost:{}ms]",classAndMethod,requertJson,JSON.toJSONString(result, SerializerFeature.DisableCircularReferenceDetect),System.currentTimeMillis()-startMini);
                return result;
            }catch (Exception e){
                traceLogManager.info("[trace-mq][{}]【request={}】【response={}】[cost:{}ms]",classAndMethod,requertJson,e.getMessage(),System.currentTimeMillis()-startMini);
                throw  e;
            }finally {
